---
title: "xMWAS_T2D_islets_omics"
author: "Jonathan Anzules & Madelyn Houser"
date: '2023-03-20'
output: 
  html_document:
    toc: true
  word_document:
    toc: true
Description: "The xMWAS_T2D_islets_omics code integrates multi-omics datasets (transcriptomics, metabolomics, and proteomics) for the analysis of islet samples from diabetic and non-diabetic individuals. The code includes data preprocessing, imputation of missing values, and log transformation. It performs multi-omics integration using the xMWAS tool to identify significant features and their associations. The analysis includes community detection, centrality measurements, and pathway enrichment analysis to reveal biological insights into Type 2 Diabetes, such as changes in metabolic pathways and gene expression networks. Visualizations are generated to illustrate the results."
---
...
tran <- read_csv("C:/Users/mecho/Documents/Hertzberg_Lab/T2D/Genomics_for_Vicki/NewAnalysis/ExpressionData_filtered.csv")

# metabolomics - prefiltered, raw values
met <- read_csv("C:/Users/mecho/Documents/Hertzberg_Lab/T2D/Metabolomics_for_Vicki/FeatureTable_justprefiltered.csv")

# proteomics - prefiltered, raw values
prot <-  read_csv("C:/Users/mecho/Documents/Hertzberg_Lab/T2D/Proteomics_for_Vicki/Analysis_Houser/ExpressionTable_justprefiltered.csv")

# class labels
key <- read_csv("C:/Users/mecho/Documents/Hertzberg_Lab/T2D/SampleKey.csv")
```
...
logmet <- read_csv("C:/Users/mecho/Documents/Hertzberg_Lab/T2D/Metabolomics_for_Vicki/FeatureTable_prefiltered.csv", skip=1)
...
output<-"C:/Users/mecho/Documents/Hertzberg_Lab/T2D/xMWAS/T2D_xMWAS_outputLOG_mettran/"
```
  
```{r xmwas_tranmet3, eval=FALSE}
...
xmwas_res<-run_xmwas(Xome_data=tran,Yome_data=logmet,Zome_data=NA,Wome_data=NA,outloc=output,
...
```{r centralitycomp_allnodes_tranmet, fig.height=4, fig.width=4}
## Read in results for controls
res.control <- read.delim("C:/Users/mecho/Documents/Hertzberg_Lab/T2D/xMWAS/T2D_xMWAS_outputLOG_mettran/Control/cluster_membership_centrality_Control_table.txt")

## Read in results for diabetic samples
res.diab <- read.delim("C:/Users/mecho/Documents/Hertzberg_Lab/T2D/xMWAS/T2D_xMWAS_outputLOG_mettran/Diabetic/cluster_membership_centrality_Diabetic_table.txt")
...
ggsave("C:/Users/mecho/Documents/Hertzberg_Lab/T2D/xMWAS/T2D_xMWAS_outputLOG_mettran/CentralityComparison_allfeatures.png", dpi=300)
```
  
```{r centralitycompbyfeature_allnodes_tranmet, fig.height=4, fig.width=8}
...
cent$`Node Type` <- factor(cent$`Node Type`, levels=c("Transcript", "Metabolite"))

ggplot(aes(x=`Node Type`, y=Centrality), data=cent) +
  geom_violin(fill="lightgray", draw_quantiles=c(0.5), scale="count", adjust=1) +
  theme_bw() +
  ylab("Eigenvector Centrality") +
  facet_wrap(~Network)


ggsave("C:/Users/mecho/Documents/Hertzberg_Lab/T2D/xMWAS/T2D_xMWAS_outputLOG_mettran/CentralityComparison_byFeatureType.png", dpi=300)

```

  
## Annotations of metabolites in networks
```{r resultssepLOG_control_tranmet, message=FALSE}
# Get annotations for putative metabolites in network

## Separate features by type
metres.control <- res.control %>% filter(!grepl("ENSG", res.control$Name)) %>% dplyr::rename(mz_rt=Name, Community_xMWAS=Cluster, Node_xMWAS=Node, Centrality.eigenvec_xMWAS=centrality_vec)
...
## Get full list of metabolites
allmet <- data.frame(rownames(met))
names(allmet) <- "mz_rt"
allmet$mz <- sapply(strsplit(as.character(allmet$mz_rt),"_"),"[[",1)
allmet$rt <- sapply(strsplit(as.character(allmet$mz_rt),"__"),"[[",2)
allmet$mz4 <- round(as.numeric(allmet$mz), digits=4)

## Get mummichog annotations for all metabolites
mumann <- read.delim("C:/Users/mecho/Documents/Hertzberg_Lab/T2D/Metabolomics_for_Vicki/Analysis_Houser_2/PathwayAnalysis/1691603859.92.mumresult/tsv/_tentative_featurematch_mumresult.tsv") %>% dplyr::rename(mz4=m.z, KEGG_ID_mummichog=id, match_form_mummichog=match_form, mz_difference_mummichog=mz_difference, Metabolite_mummichog=name, pathway_mummichog=pathway)

## Add mummichog annotations to full list of metabolites
metres.control.ann <- left_join(allmet, mumann, by="mz4")

## Get t test selected features information
### Remove columns which would be duplicated upon merging
selfeat <- read_csv("C:/Users/mecho/Documents/Hertzberg_Lab/T2D/Metabolomics_for_Vicki/Analysis_Houser_2/Annotation/ttestraw_merged_annotation.csv") %>% dplyr::select(-(KEGG_ID_mummichog:pathway_mummichog)) %>% dplyr::rename(' -log10p'='-log10(p)')
...
write_csv(metres.control.ann, "C:/Users/mecho/Documents/Hertzberg_Lab/T2D/xMWAS/T2D_xMWAS_outputLOG_mettran/Control/MetaboliteResults_Control_withAnnotation.csv")

```
  
```{r resultssepLOG_diab_tranmet, message=FALSE}
# Get annotations for putative metabolites in network

## Separate features by type
metres.diab <- res.diab %>% filter(!grepl("ENSG", res.diab$Name)) %>% dplyr::rename(mz_rt=Name, Community_xMWAS=Cluster, Node_xMWAS=Node, Centrality.eigenvec_xMWAS=centrality_vec)
...
write_csv(metres.diab.ann, "C:/Users/mecho/Documents/Hertzberg_Lab/T2D/xMWAS/T2D_xMWAS_outputLOG_mettran/Diabetic/MetaboliteResults_Diabetic_withAnnotation.csv")

```
  
### Centrality comparison of metabolite communities
```{r compCentralityLOG_tranmet}
...
```{r commassignmetsLOG_ann_tranmet}
# Get annotations for putative metabolites in network
...
```{r commmet_differencesLOG_tranmet}
...
print("Overall, to what communities in the control network are metabolites assigned to community 7 in diabetic samples assigned?")
metres.all.ann %>% filter(Community.Diabetic_xMWAS=="7") %>% dplyr::select(Community.Control_xMWAS) %>% table(useNA = "ifany")
```
  
  
## Results: xMWAS transcriptomics and metabolomics - metabolites
  
#### **`r res.control %>% filter(!grepl("ENSG", res.control$Name)) %>% nrow()` metabolomic features (the maximum permitted equivalent to top 30% of the metabolites based on relative standard deviation) and `r res.control %>% filter(grepl("ENSG", res.control$Name)) %>% nrow()` transcripts were selected in the sPLS analysis for the control network. `r res.diab %>% filter(!grepl("ENSG", res.diab$Name)) %>% nrow()` metabolomic features (the maximum permitted equivalent to top 30% of the metabolites based on relative standard deviation) and `r res.diab %>% filter(grepl("ENSG", res.diab$Name)) %>% nrow()` transcripts were selected in the sPLS analysis for the diabetic network.**  
...
#### **Control community 1 contained `r metres.control %>% filter(Community_xMWAS=="1") %>% nrow()` metabolites and `r tranres.control %>% filter(Community_xMWAS=="1") %>% nrow()` transcripts.**  
#### **Control community 2 contained `r metres.control %>% filter(Community_xMWAS=="2") %>% nrow()` metabolites and `r tranres.control %>% filter(Community_xMWAS=="2") %>% nrow()` transcripts.**  
#### **Control community 3 contained `r metres.control %>% filter(Community_xMWAS=="3") %>% nrow()` metabolites and `r tranres.control %>% filter(Community_xMWAS=="3") %>% nrow()` transcripts.**  
#### **Control community 4 contained `r metres.control %>% filter(Community_xMWAS=="4") %>% nrow()` metabolites and `r tranres.control %>% filter(Community_xMWAS=="4") %>% nrow()` transcripts.**  
#### **Control community 5 contained `r metres.control %>% filter(Community_xMWAS=="5") %>% nrow()` metabolites and `r tranres.control %>% filter(Community_xMWAS=="5") %>% nrow()` transcripts.**  
#### **Control community 6 contained `r metres.control %>% filter(Community_xMWAS=="6") %>% nrow()` metabolites and `r tranres.control %>% filter(Community_xMWAS=="6") %>% nrow()` transcripts.**  
  
#### **Diabetic community 1 contained `r metres.diab %>% filter(Community_xMWAS=="1") %>% nrow()` metabolites and `r tranres.diab %>% filter(Community_xMWAS=="1") %>% nrow()` transcripts.**  
#### **Diabetic community 2 contained `r metres.diab %>% filter(Community_xMWAS=="2") %>% nrow()` metabolites and `r tranres.diab %>% filter(Community_xMWAS=="2") %>% nrow()` transcripts.**  
#### **Diabetic community 3 contained `r metres.diab %>% filter(Community_xMWAS=="3") %>% nrow()` metabolites and `r tranres.diab %>% filter(Community_xMWAS=="3") %>% nrow()` transcripts.**  
#### **Diabetic community 4 contained `r metres.diab %>% filter(Community_xMWAS=="4") %>% nrow()` metabolites and `r tranres.diab %>% filter(Community_xMWAS=="4") %>% nrow()` transcripts.**  
#### **Diabetic community 5 contained `r metres.diab %>% filter(Community_xMWAS=="5") %>% nrow()` metabolites and `r tranres.diab %>% filter(Community_xMWAS=="5") %>% nrow()` transcripts.**  
#### **Diabetic community 6 contained `r metres.diab %>% filter(Community_xMWAS=="6") %>% nrow()` metabolites and `r tranres.diab %>% filter(Community_xMWAS=="6") %>% nrow()` transcripts.**  
#### **Diabetic community 7 contained `r metres.diab %>% filter(Community_xMWAS=="7") %>% nrow()` metabolites and `r tranres.diab %>% filter(Community_xMWAS=="7") %>% nrow()` transcripts.**  
 
  
#### **The greatest connectivity (eigenvector centrality) was found in community 1 for control samples, followed by 3 then 4. The greatest connectivity was found in community 2 for diabetic samples, followed by 4, 1, 5, then 7.**  
  
#### **While similar numbers of communities were identified in diabetic and non-diabetic control networks, there was little correspondence among the communities. The majority of features included in the diabetic networks were not included in the control networks, and vice versa. The greatest overlap was between community 2 in controls and community 3 in diabetic samples (`r metres.all.ann %>% filter(Community.Control_xMWAS=="2") %>% filter(Community.Diabetic_xMWAS=="3") %>% nrow()` out of `r metres.all.ann %>% filter(Community.Control_xMWAS=="2") %>% nrow()` [`r round((metres.all.ann %>% filter(Community.Control_xMWAS=="2") %>% filter(Community.Diabetic_xMWAS=="3") %>% nrow())/(metres.all.ann %>% filter(Community.Control_xMWAS=="2") %>% nrow())*100, digits=1)`%] features assigned to community 2 in controls were assigned to community 3 in the diabetic network) followed by the overlap between community 3 in controls and community 6 in diabetic samples (`r metres.all.ann %>% filter(Community.Control_xMWAS=="3") %>% filter(Community.Diabetic_xMWAS=="6") %>% nrow()` out of `r metres.all.ann %>% filter(Community.Control_xMWAS=="3") %>% nrow()` [`r round((metres.all.ann %>% filter(Community.Control_xMWAS=="3") %>% filter(Community.Diabetic_xMWAS=="6") %>% nrow())/(metres.all.ann %>% filter(Community.Control_xMWAS=="3") %>% nrow())*100, digits=1)`%] features assigned to community 3 in controls were assigned to community 6 in the diabetic network).**  
...
```{r deltaCentralityLOG_tranmet}
deltcent <- read.delim("C:/Users/mecho/Documents/Hertzberg_Lab/T2D/xMWAS/T2D_xMWAS_outputLOG_mettran/class-wise_centrality_matrix.txt")

# Ensure that column headers are in correct location
if(names(deltcent)[5]=="X"){
  names(deltcent)[2:5] <- names(deltcent)[1:4]
  names(deltcent)[1] <- "mz_rt"
}

# Separate data sets
met.deltcent <- deltcent %>% filter(!grepl("ENSG", deltcent$mz_rt))
...
```{r deltaCentralityLOG_ann_tranmet}
# Get annotations for putative metabolites in network
...
write_csv(metres.all.ann, "C:/Users/mecho/Documents/Hertzberg_Lab/T2D/xMWAS/T2D_xMWAS_outputLOG_mettran/Results_AllMetabolites_withAnnotation.csv")
...
```{r commmet_deltaCentrality_differencesLOG_tranmet}
# Explore the differences between the metabolites in the control and diabetic networks
...
print("How many of these features assigned to community 2 in diabetic samples are being assigned to community 1 in control samples?")
sigmetres.all.ann %>% filter(Community.Diabetic_xMWAS=="2") %>% filter(Community.Control_xMWAS=="1") %>% nrow()

print("How many of these features assigned to community 1 in control samples are being assigned to community 2 in diabetic samples?")
sigmetres.all.ann %>% filter(Community.Control_xMWAS=="1") %>% filter(Community.Diabetic_xMWAS=="2") %>% nrow()

print("To what communities are these features assigned to community 2 in diabetic samples being assigned in control samples?")
sigmetres.all.ann %>% filter(Community.Diabetic_xMWAS=="2") %>% dplyr::select(Community.Control_xMWAS) %>% table(useNA = "ifany")
...
```{r deltaCentrality_mummichogformat_tranmet}
...
write.table(mumft.deltcent, "C:/Users/mecho/Documents/Hertzberg_Lab/T2D/xMWAS/T2D_xMWAS_outputLOG_mettran/DeltaCentrality_Pathways/Mummichog_deltcent_input.txt", col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")  
```
  
#### Pathway analysis with mummichog
```{r mummichog_deltaCentrality_tranmet}
# Run with Python using Anaconda 
 
# Open Anaconda Prompt 
 
# In main user directory, create python 2.7 environment 
## >conda create --name py2 python=2.7 
 
# navigate to folder you want to work in for mummichog analysis and activate python environment 
## >activate py2 
 
# Install mummichog1 in that environment 
## >pip install mummichog1 
 
# To see documentation for mummichog1: 
## > mummichog1 
### Output: 
...
# > cd C:\Users\mecho\Documents\Hertzberg_Lab\T2D\xMWAS\T2D_xMWAS_outputLOG_mettran\DeltaCentrality_Pathways

# > mummichog1 -fC:\Users\mecho\Documents\Hertzberg_Lab\T2D\xMWAS\T2D_xMWAS_outputLOG_mettran\DeltaCentrality_Pathways\Mummichog_deltcent_input.txt -n human_mfn -o mumresult -m dpj -u 10 -p 1000 -z True -c 0.05 
```
...
```{r refinePathways_deltaCentrality_tranmet, message=FALSE}
# Import pathway analysis file
path <- read.delim("C:/Users/mecho/Documents/Hertzberg_Lab/T2D/xMWAS/T2D_xMWAS_outputLOG_mettran/DeltaCentrality_Pathways/1700236583.32.mumresult/tsv/mcg_pathwayanalysis_mumresult.tsv")
...
write_csv(path, "C:/Users/mecho/Documents/Hertzberg_Lab/T2D/xMWAS/T2D_xMWAS_outputLOG_mettran/DeltaCentrality_Pathways/1700236583.32.mumresult/tsv/mcg_pathwayanalysis_mumresult.csv")

# Clean import of trimmed pathway analysis file
path <- read_csv("C:/Users/mecho/Documents/Hertzberg_Lab/T2D/xMWAS/T2D_xMWAS_outputLOG_mettran/DeltaCentrality_Pathways/1700236583.32.mumresult/tsv/mcg_pathwayanalysis_mumresult.csv")
...
write_csv(path, "C:/Users/mecho/Documents/Hertzberg_Lab/T2D/xMWAS/T2D_xMWAS_outputLOG_mettran/DeltaCentrality_Pathways/Significant_Pathways_deltaCentrality.csv")
```
   
   
   
#### Plot pathway analysis results
```{r pathgraph_deltaCentrality_tranmet, fig.width=9, fig.height=2.5}
...
ggsave("C:/Users/mecho/Documents/Hertzberg_Lab/T2D/xMWAS/T2D_xMWAS_outputLOG_mettran/DeltaCentrality_Pathways/pathfig_deltaCentrality.png")
```
  
#### Determine direction of metabolite change from one group to another for significant pathways
```{r pathdirection_deltaCentrality_tranmet, message=FALSE}
...
colnames(overlapmz) <- path$pathway

# Get annotation information just for overlap features
overlapmzls <- c(overlapmz)
overann <- overann[overann$`m/z` %in% overlapmzls,]
write_csv(overann, "C:/Users/mecho/Documents/Hertzberg_Lab/T2D/xMWAS/T2D_xMWAS_outputLOG_mettran/DeltaCentrality_Pathways/overlap_ann_deltaCentrality.csv")
...
```{r plotdirfeatures_deltaCentrality_tranmet, fig.height=5, fig.width=27}
...
fcplot
ggsave("C:/Users/mecho/Documents/Hertzberg_Lab/T2D/xMWAS/T2D_xMWAS_outputLOG_mettran/DeltaCentrality_Pathways/dcfeatures_in_pathways_deltaCentrality.png")
```
  
```{r replotpathfig_deltaCentrality_tranmet, fig.width=9, fig.height=2.5}
...
pathfig <- ggplot(pathg, aes(x=reorder(Pathways, neglogpvalue), y=neglogpvalue, fill=Direction)) +
  ...
  scale_fill_manual(values=c("red", "gray", "black")) +
  coord_flip() +
  scale_x_discrete(name="Pathways Associated with\nHigher Delta Centrality") +
  ylab("-log(p value)") +
  theme(axis.text.x = element_text(face="bold", size=10, angle=0),
...
ggsave("C:/Users/mecho/Documents/Hertzberg_Lab/T2D/xMWAS/T2D_xMWAS_outputLOG_mettran/DeltaCentrality_Pathways/pathfig_wcolor_deltaCentrality.png")
```
...
#### **When we performed pathway analysis with these `r nrow(sigmet.deltcent)` metabolomic features, one pathway with overlap >=3 and padj < 0.05 was identified: glycosphingolipid metabolism. Tentative annotations for features assigned to this pathway were galactosylsphingosine or psychosine, psychosine sulfate, and FADH2. Of these, psychosine sulfate had higher centrality in the diabetic network while FADH2 and galactosylsphingosine or psychosine had higher centrality in the control network.**  
  
#### **We also observed that among the metabolomic features with delta centrality >0.1, the vast majority included in the control network were from community 1 while the vast majority included in the diabetic network were from community 2. This does not reflect a simple difference in community label between the networks, as only `r sigmetres.all.ann %>% filter(Community.Diabetic_xMWAS=="2") %>% filter(Community.Control_xMWAS=="1") %>% nrow()` metabolomic features with high delta centrality assigned to community 2 in diabetic samples were assigned to community 1 in control samples.**  
...
```{r tranannLOG_control_tranmet, message=FALSE}
# Get annotations for transcripts in network

## Get full list of transcripts
alltran <- data.frame(rownames(tran))
names(alltran) <- "EnsembleID"

## Get gene names for all transcripts
tranann <- read_csv("C:/Users/mecho/Documents/Hertzberg_Lab/T2D/Genomics_for_Vicki/NewAnalysis/ExpressionData_filtered.csv") %>% dplyr::rename(GeneName=external_gene_name) %>% dplyr::select(EnsembleID, GeneName)


## Add transcript names to full list of transcripts
tranres.control.ann <- left_join(alltran, tranann, by="EnsembleID")

## Get DGE selected features information
seltran <- read_csv("C:/Users/mecho/Documents/Hertzberg_Lab/T2D/Genomics_for_Vicki/NewAnalysis/DGEdataframe_filtered.csv") %>% dplyr::rename(EnsembleID=Gene_names, DtoC.log2FC=log2FC, DGE.pvalue=pvalue, DGE.rank=rank) %>% dplyr::select(EnsembleID:DGE.rank) 
...
write_csv(tranres.control.ann, "C:/Users/mecho/Documents/Hertzberg_Lab/T2D/xMWAS/T2D_xMWAS_outputLOG_mettran/Control/TranscriptResults_Control_withAnnotation.csv")

```
  
```{r tranannLOG_diab_tranmet, message=FALSE}
# Get annotations for transcripts in network
...
write_csv(tranres.diab.ann, "C:/Users/mecho/Documents/Hertzberg_Lab/T2D/xMWAS/T2D_xMWAS_outputLOG_mettran/Diabetic/TranscriptResults_Diabetic_withAnnotation.csv")

```
  
### Centrality comparison of transcript communities
```{r compCentrality_tran_LOG_tranmet}
...
```{r commassignmets_tran_LOG_ann_tranmet}
# Get annotations for transcripts in network
...
```{r trancomm_differencesLOG_tranmet}
...
print("Overall, to what communities in the control network are transcripts assigned to community 7 in diabetic samples assigned?")
tranres.all.ann %>% filter(Community.Diabetic_xMWAS=="7") %>% dplyr::select(Community.Control_xMWAS) %>% table(useNA = "ifany")
```
  
  
## Results: xMWAS transcriptomics and metabolomics - transcripts
    
#### **In the control samples network, metabolites had higher centrality scores than transcripts on average. In the diabetic samples network, transcripts had higher centrality scores than metabolites.**  
    
#### **The greatest connectivity (eigenvector centrality) was found in community 2 for diabetic samples, followed by 1, 4, 5, 7, 3, and then 6 - similar to the findings for metabolites. Centrality scores above 0 for transcripts in the control network were rarely observed.**  
  
#### **There was more overlap between the transcripts included in the two networks than there was between the metabolites. Community 1 in the control network contained the majority of the transcripts in the control analysis (`r tranres.all.ann %>% filter(Community.Control_xMWAS=="1") %>% nrow()` out of `r res.control %>% filter(grepl("ENSG", res.control$Name)) %>%  nrow()`, `r round(((tranres.all.ann %>% filter(Community.Control_xMWAS=="1") %>% nrow())/(res.control %>% filter(grepl("ENSG", res.control$Name)) %>%  nrow())*100), digits=1)`%). With the exception of community 5, a notable percentage of transcripts in the other communities in the diabetic sample network were included in community 1 in the control network.**  
...
```{r tran_deltaCentralityLOG_tranmet}
# Separate data sets
tran.deltcent <- deltcent %>% filter(grepl("ENSG", deltcent$mz_rt)) %>% dplyr::rename(EnsembleID=mz_rt)
...
```{r tran_deltaCentralityLOG_ann_tranmet}
# Get annotations for transcripts in network
...
write_csv(tranres.all.ann, "C:/Users/mecho/Documents/Hertzberg_Lab/T2D/xMWAS/T2D_xMWAS_outputLOG_mettran/Results_Alltranscripts_withAnnotation.csv")
...
```{r commtran_deltaCentrality_differencesLOG_tranmet}
# Explore the differences between the transcripts in the control and diabetic networks
...
print("How many of these features assigned to community 2 in diabetic samples are being assigned to community 1 in control samples?")
sigtranres.all.ann %>% filter(Community.Diabetic_xMWAS=="2") %>% filter(Community.Control_xMWAS=="1") %>% nrow()

print("How many of these features assigned to community 1 in control samples are being assigned to community 2 in diabetic samples?")
sigtranres.all.ann %>% filter(Community.Control_xMWAS=="1") %>% filter(Community.Diabetic_xMWAS=="2") %>% nrow()

print("To what communities are these features assigned to community 2 in diabetic samples being assigned in control samples?")
sigtranres.all.ann %>% filter(Community.Diabetic_xMWAS=="2") %>% dplyr::select(Community.Control_xMWAS) %>% table(useNA = "ifany")
...
```{r tran_deltaCentrality_GSEAformat_tranmet}
...
```{r gmtGO_deltaCentrality_tranmet}
# Import .gmt file with pathway information by gene symbol (Gene sets derived from the GO ontology)
## Obtained from GSEA 3-6-2023 http://www.gsea-msigdb.org/gsea/msigdb/collections.jsp
go <- gmtPathways("C:/Users/mecho/Documents/Hertzberg_Lab/T2D/Proteomics_for_Vicki/Analysis_Houser/Pathways/c5.go.v2023.1.Hs.symbols.gmt")
```
  
```{r pathfgseaGO_deltaCentrality_tranmet}
set.seed(08162023)
...
```{r gseaGO_deltaCentrality_pathsel_tranmet}
# write table
fwrite(fgseaRes, file="C:/Users/mecho/Documents/Hertzberg_Lab/T2D/xMWAS/T2D_xMWAS_outputLOG_mettran/DeltaCentrality_Pathways/Transcripts/fgseaRes_GO.txt", sep="\t", sep2=c("", " ", ""))


# read table
pathtab <- read.delim("C:/Users/mecho/Documents/Hertzberg_Lab/T2D/xMWAS/T2D_xMWAS_outputLOG_mettran/DeltaCentrality_Pathways/Transcripts/fgseaRes_GO.txt")
...
topup$pathway <- topup$pathway %>% str_replace("GO.+?_", "") %>% str_replace_all("_", " ")

# label whether pathways are more or less active
topup$Direction[1:10] <- "Up"

topup <- topup[order(topup$pval),]
topup <- topup %>% filter(pval<=0.05) %>% filter(!is.na(pathway))


# Write cleaned table
write_csv(topup, "C:/Users/mecho/Documents/Hertzberg_Lab/T2D/xMWAS/T2D_xMWAS_outputLOG_mettran/DeltaCentrality_Pathways/Transcripts/Top_GSEA_Pathways_GO.csv")
```
  
#### Plot pathway analysis results
  
#### Significant pathways (padj < 0.1)
```{r sigpathgraph_deltaCentrality_GO_tranmet, fig.width=11, fig.height=6}
# Summarize pathway information and create negative log p value variable for graphing
...
pathg <- pathg %>% mutate(neglogpvalue=-log10(pval))
  ...
  ggsave("C:/Users/mecho/Documents/Hertzberg_Lab/T2D/xMWAS/T2D_xMWAS_outputLOG_mettran/DeltaCentrality_Pathways/Transcripts/pathsigfig_GO.png")
...
```{r gmtKEGG_deltaCentrality_tranmet}
# Import .gmt file with pathway information by gene symbol (Gene sets derived from the KEGG pathway database)
## Obtained from GSEA 3-6-2023 http://www.gsea-msigdb.org/gsea/msigdb/collections.jsp
...
```{r pathfgseaKEGG_deltaCentrality_tranmet}
set.seed(08162023)
...
```{r gseaKEGG_deltaCentrality_pathsel_tranmet}
# write table
fwrite(fgseaRes, file="C:/Users/mecho/Documents/Hertzberg_Lab/T2D/xMWAS/T2D_xMWAS_outputLOG_mettran/DeltaCentrality_Pathways/Transcripts/fgseaRes_KEGG.txt", sep="\t", sep2=c("", " ", ""))

# read table
pathtab <- read.delim("C:/Users/mecho/Documents/Hertzberg_Lab/T2D/xMWAS/T2D_xMWAS_outputLOG_mettran/DeltaCentrality_Pathways/Transcripts/fgseaRes_KEGG.txt")
...
# Clean up pathway names
topup$pathway <- topup$pathway %>% str_replace("KEGG_", "") %>% str_replace_all("_", " ")
...
# Write cleaned table
write_csv(topup, "C:/Users/mecho/Documents/Hertzberg_Lab/T2D/xMWAS/T2D_xMWAS_outputLOG_mettran/DeltaCentrality_Pathways/Transcripts/Top_GSEA_Pathways_KEGG.csv")
```
  
#### Plot pathway analysis results
  
#### Significant pathways (padj < 0.1)
```{r sigpathgraph_deltaCentrality_KEGG_tranmet, fig.width=11, fig.height=2.5}
# Summarize pathway information and create negative log p value variable for graphing
# Black = delta centrality higher in diabetic network
pathg <- topup %>% filter(padj<=0.1) 
pathg <- pathg %>% mutate(neglogpvalue=-log10(pval))
  ...
  ggsave("C:/Users/mecho/Documents/Hertzberg_Lab/T2D/xMWAS/T2D_xMWAS_outputLOG_mettran/DeltaCentrality_Pathways/Transcripts/pathsigfig_KEGG.png")
...
#### **`r nrow(sigtran.deltcent)` transcripts had a difference in eigenvector centrality score >0.1 between diabetic sample and non-diabetic control sample networks (delta centrality). Transcript centrality was higher in the diabetic network than the control, so these were the transcripts with the greatest increase in connectivity in diabetic vs control networks.**  
  
#### **When we performed gene set enrichment analysis (GSEA) with these `r nrow(sigtran.deltcent)` transcripts with higher centrality in the diabetic network, the top 10 GO biological processes associated with them were cytoplasmic sequestering of transcription factor, atrioventricular canal development, non-membrane-spanning protein tyrosine phosphatase activity, negative regulation of interleukin 6 production, positive regulation of lymphocyte migration, protein serine kinase activity, protein serine threonine kinase activity, response to muramyl dipeptide, positive regulation of T cell migration, and negative regulation of antigen receptor-mediated signaling pathway. The KEGG pathways significantly (FDR<0.1) associated with these transcripts were GNRH signaling and intestinal immune network for IGA production.**  
  
#### **We also observed that among the transcripts with delta centrality >0.1 (`r nrow(sigtran.deltcent)`), the vast majority (`r sigtranres.all.ann %>% filter(Community.Diabetic_xMWAS=="2") %>% nrow()`, `r round(((sigtranres.all.ann %>% filter(Community.Diabetic_xMWAS=="2") %>% nrow())/(nrow(sigtran.deltcent))*100), digits=1)`%) were from community 2 in the diabetic network.**  
...
```{r cleanup_tranmet}
rm(comms, deltcent, fcplot, i, met.deltcent, metres.all.ann, metres.control, metres.control.ann, metres.dcdir, metres.diab, metres.diab.ann, mintran, mumft.control, mumft.deltcent, overann, overann.ann, overlapmz, overlapmzls, path, path2, pathfig, pathg, pathoverlap2, pathtbl2, res.control, res.diab, sigmet.deltcent, sigmetres.all.ann, tranres.control, tranres.control.ann, tranres.diab, tranres.diab.ann)
```
  
## Run xMWAS
```{r setoutput_tranmetprot}
output<-"C:/Users/mecho/Documents/Hertzberg_Lab/T2D/xMWAS/T2D_xMWAS_output_mettranprot/"
```
